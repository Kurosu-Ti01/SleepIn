...
### 5.4 CSV 场景下的 UseCase 串联

除了课程表 CSV（`ImportCsvUseCase` / `ExportCsvUseCase`），当前还包含作息表 CSV：

- `ImportScheduleCsvUseCase`：解析 CSV 后调用 `SaveScheduleUseCase`，并强制创建新作息表。
- `ExportScheduleCsvUseCase`：读取 `GetScheduleDetailUseCase` 结果并生成 CSV 文本。
...
- 下一篇建议：`docs/SleepIn-Docs/docs/dev/3.business/3.ui-viewmodel-compose.md`
- 如果想看文件输入输出链路：`docs/SleepIn-Docs/docs/dev/3.business/4.csv-integration.md`
