...
/**
 * Shared reminder execution logic used by alarm receiver and worker fallback.
 */
object CourseReminderNotificationExecutor {
    ...
    suspend fun execute(
        ...
        val activeTimetable = app.getActiveTimetableUseCase().first() ?: return
        val weekInfo = calculateSemesterWeekInfo(
            startDate = activeTimetable.startDate,
            totalWeeks = activeTimetable.totalWeeks,
            today = now.toLocalDate()
        )
        if (weekInfo.progress != SemesterProgress.IN_PROGRESS) return

        val currentWeek = weekInfo.currentWeek ?: return
        ...
        return courses
            .flatMap { aggregate ->
                aggregate.sessions.mapNotNull { session ->
                    ...
                    val latestTriggerAt = minOf(startAt, reminderAt.plusMinutes(toleranceMinutes))

                    if (now.isBefore(reminderAt) || now.isAfter(latestTriggerAt)) {
    ...
    private fun loadDedupeSet(context: Context, nowEpochMillis: Long): MutableSet<String> {
    ...
    private fun saveDedupeSet(context: Context, set: Set<String>) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    ...
    val PERIODIC_TOLERANCE_MINUTES = 15L
}
